...
interface PinnedProjectTypeProps {
  projectType: string;
  currentStep: 'type' | 'address' | 'details';
}
      ...
      <CardContent>
        ...
        <div className="space-y-1">
          ...
          <div className="flex space-x-1">
            <div className={`h-1 w-1/3 rounded-full ${currentStep === 'type' ? 'bg-primary' : 'bg-primary/30'}`} />
            <div className={`h-1 w-1/3 rounded-full ${currentStep === 'address' ? 'bg-primary' : 'bg-primary/30'}`} />
            <div className={`h-1 w-1/3 rounded-full ${currentStep === 'details' ? 'bg-primary' : 'bg-primary/30'}`} />
          </div>
        </div>
      </CardContent>
    </Card>
